...
def flags_maker(end=False, syn=False, rst=False, ack=False):
    flags = 0
...
def headers_maker(source_port=0, destination_port=0, sequence_number=0, ack=0,
...
def split_package(pack: bytes):
    ...
    # for i in range(50):
    #     print(i, int.from_bytes(pack[2+i:4+i], byteorder='little', signed=True))
    return dic, pack[20+of:]


def make_package(data: bytes, **dic) -> bytes:
...
empty_header = {'source_port': b'\x00\x00',
